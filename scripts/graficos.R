...
par(mfrow = c(2,2))
with(dados, hist(as.numeric(dt.diag), main = "Tempo de diagnóstico", xlab = ""))
corte <- 30; barplot(table(dados$dt.diag > corte), names.arg =  c(paste("<", corte, "dias"), paste(">", corte, "dias")))
corte <- 60; barplot(table(dados$dt.diag > corte), names.arg =  c(paste("<", corte, "dias"), paste(">", corte, "dias")))
corte <- 120; barplot(table(dados$dt.diag > corte), names.arg =  c(paste("<", corte, "dias"), paste(">", corte, "dias")))
dev.off()
